...
- `Sub`: Pops the two topmost values from the stack, subtracts the first popped value from the second, then pushes the result onto the stack.
- `Mult`: Pops the two topmost values from the stack, multiplies them together, then pushes the result onto the stack.
- `Div`: Pops the two topmost values from the stack, divides the second popped value by the first, then pushes the result onto the stack.
...
- `LessThan`: Pops the two topmost values on from the stack, checks whether the second popped value is less than the first, then pushes the result onto the stack as a bool value.
