...
| `map` | Maps the values of a list to a new set of values using a transform function, **returning a new list**. | `source`: The source list to transform.<br/>`transform`: A function to apply to each element of the list, where the value returned from the function will be the value at the same index in the new list. | A new list with the same length as the source list with every element transformed using the transform function. | `0x185` |
